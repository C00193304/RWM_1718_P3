...
public:
	...
	void Fireworks::Render(Renderer &r)
	{
		for (int i = 0; i < particles.size(); i++)
		{
			Rect rect(particles.at(i).body->GetPosition().x, particles.at(i).body->GetPosition().y, particles.at(i).width, particles.at(i).height);
